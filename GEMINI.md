...
## Project Information
- **Target URLs** (vars.SN_PDI_URL, comma-separated):
  - https://dev380301.service-now.com (global SN_USERNAME/SN_PASSWORD; wake: global SN_DEV_*)
  - https://dev400970.service-now.com (per-instance login; wake: global SN_DEV_* = go41@naver.com)
  - https://dev404356.service-now.com (per-instance login; wake: per-instance dev acct aaronisdjyalu@gmail.com)
  - https://dev404357.service-now.com (per-instance login; wake: per-instance dev acct msookim@lgcns.com)
- **Repository**: https://github.com/djyalu/servicenow_pdi_autologin.git
- **Schedule**: Every 2 hours (`0 */2 * * *`)
- **Post-wake retry**: if an instance was hibernating, it is woken, logged in, then logged in again 30 minutes later (in-job)
- **Notification**: go41@naver.com (on success/failure)
- **Credentials**: Managed via Github Actions Secrets/Variables
  - `SN_USERNAME` / `SN_PASSWORD`: default standard-login account
  - `SN_CREDENTIALS`: JSON map of host -> {username, password} for per-instance logins
  - `SN_DEV_USERNAME` / `SN_DEV_PASSWORD`: developer.servicenow.com account used to wake hibernating PDIs
...
- **2026-06-13**: 신규 인스턴스 `dev400970.service-now.com` 추가. 인스턴스별 자격증명을 지원하도록 `SN_CREDENTIALS`(host->creds JSON Secret) 도입하여 `main.py` 고도화. (분석 설계자, 백엔드 개발자)
- **2026-06-13**: 휴면 깨우기(SSO)는 개발자 계정(`SN_DEV_USERNAME`/`SN_DEV_PASSWORD`), 표준 로그인은 인스턴스별 계정을 사용하도록 자격증명 분리. (아키텍트, 백엔드 개발자)
- **2026-06-13**: ServiceNow의 지속 연결로 인한 `networkidle` 타임아웃을 비치명적으로 처리하고, 휴면 인스턴스 깨우기를 폴링/재시도 + 쿠키배너/Wake버튼 처리로 견고화. (품질 관리자, 백엔드 개발자)
- **2026-06-13**: 두 인스턴스(dev380301, dev400970) 모두 GitHub Actions 자동 로그인 테스트 성공 확인 및 배포 완료. (테스터, 배포담당자, Project Manager)
- **2026-06-13**: 신규 인스턴스 `dev404356.service-now.com` 추가. 인스턴스마다 깨우기용 개발자 계정이 다를 수 있어 `SN_CREDENTIALS` 항목에 `dev_username`/`dev_password` 필드(per-instance 깨우기 계정) 지원 추가. (분석 설계자, 백엔드 개발자)
- **2026-06-13**: 세 인스턴스(dev380301, dev400970, dev404356) 모두 자동 로그인 테스트 성공 확인 및 배포 완료. (테스터, 배포담당자, Project Manager)
- **2026-06-13**: 신규 인스턴스 `dev404357.service-now.com` 추가(깨우기 계정 msookim@lgcns.com). 네 인스턴스 모두 자동 로그인 테스트 성공 확인 및 배포 완료. (백엔드 개발자, 테스터, 배포담당자)
- **2026-06-13**: 실행 주기를 3시간에서 2시간(`0 */2 * * *`)으로 단축. 휴면을 깨운 경우 첫 로그인 성공 후 30분 뒤 한 번 더 로그인하도록 `do_login()` 함수화 및 재로그인 로직 추가. (아키텍트, 백엔드 개발자, 배포담당자)
